...
import { GroupProduct, KategoriAktif } from './group-product.model';
...
@Injectable()
export class GroupProductService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/GroupProduct';
  ...
  save(data: GroupProduct): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: GroupProduct): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
