...
import {Search} from './sasaran.model';
...
export class SasaranComponent implements AfterViewInit {
  ...
  constructor(
    ...
    pagination(){
      ...
      this.sasaranService.getLimit(this.offset,this.limitVal)
      .subscribe(resp => {
        console.log(resp);
        this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.sasaranService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.sasaranService.getAll().subscribe(resp => {
      this.sasaranService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
            return element.sasaran.id === value.id && element.menuTab.activationCode === 'Y';
          });
  ...
  searching() {
    this.sasaranService.getByName(this.search.sasaran).subscribe(resp => {
      this.sasaranService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.sasaran !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.sasaran).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.sasaranService.getByData(this.search).subscribe(resp => {
      this.sasaranService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.sasaranService.getById(this.target).subscribe(role => {
      this.sasaranService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
