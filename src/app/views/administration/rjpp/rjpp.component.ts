...
@Component({
  selector: 'app-rjpp',
  templateUrl: './rjpp.component.html',
  styleUrls: ['./rjpp.component.scss'],
  providers: [RjppService]
})
// export class RjppComponent implements OnInit {
...
// }
export class RjppComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(RjppService) private rjppService: RjppService
  ) {
    this.search = {
      rjppid: '',
      periode: '',
  ...
  pagination(){
    ...
    this.rjppService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.rjppService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.rjppService.getAll().subscribe(resp => {
      this.rjppService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.rjppService.getByName(this.search.rjppid).subscribe(resp => {
      this.rjppService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.rjppid !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.rjppid).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.rjppService.getByData(this.search).subscribe(resp => {
      this.rjppService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.rjppService.getById(this.target).subscribe(role => {
      this.rjppService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
