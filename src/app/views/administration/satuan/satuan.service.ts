...
import { Satuan, KategoriAktif } from './satuan.model';
...
export class SatuanService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/Satuan';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&satuanid=' + id)
  }
  ...
  save(data: Satuan): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: Satuan): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&satuanid=' + data.satuanid)
  }
